...
export async function createLocalOAuthApp(options: LocalOAuthAppOptions): Promise<LocalOAuthAppResult> {
  ...
  function parseCookies(req: IncomingMessage): Record<string, string> {
    ...
    // ⚡ Bolt: Optimize parsing by removing `.split(';')` to avoid array allocations.
    // We use indexOf and manual index trimming to extract key-value pairs faster.
    let start = 0
    ...
    while (start < len) {
      ...
      if (eqIdx !== -1 && eqIdx < end) {
        // Optimization: Use `substring` and native `trim()` to avoid multiple
        // string allocations without the fragility of manual index manipulation.
        const k = header.substring(start, eqIdx).trim()
        const v = header.substring(eqIdx + 1, end).trim()
        if (k.length > 0) {
