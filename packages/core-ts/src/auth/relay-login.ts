...
function renderLoginBody(next: string, error?: string): string {
  ...
  const errorHtml = error
    ? `\n                <div id="login-error" class="status-box error" role="alert" style="display: block; margin-bottom: 1rem;">${escapeHtml(error)}</div>`
    : ''
  const ariaInvalid = error ? ' aria-invalid="true" aria-errormessage="login-error"' : ''
  ...
  return `    <div class="container">
        <div class="card">
            ...
            <form method="POST" action="/login" novalidate>${errorHtml}
...
export async function loginGetHandler(
  ...
  res.send(renderFormShell('Relay login', renderLoginBody(next)))
}
...
export async function loginPostHandler(
  ...
  if (!configuredPassword || !timingSafeEqual(password, configuredPassword)) {
    bumpFail(ip)
    res.status(401).send(renderFormShell('Relay login', renderLoginBody(next, 'Invalid password.')))
    return
  }
  clearFail(ip)
