...
describe('tryOpenBrowser', () => {
  ...
  describe('PowerShell execution', () => {
    ...
    it('uses powershell.exe with -Command and separate Base64 argument on win32', async () => {
      Object.defineProperty(process, 'platform', { value: 'win32' })
      vi.mocked(execFile).mockImplementation((...args: any[]) => {
        const _options = args[2]
        const cb = args[3]
      ...
      const commandIdx = args.indexOf('-Command') + 1
      const commandStr = args[commandIdx]
      const base64Arg = args[commandIdx + 1]

      const base64Url = Buffer.from(url, 'utf8').toString('base64')
      expect(base64Arg).toBe(base64Url)
      expect(commandStr).toContain(
        '[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($args[0])'
      )
      expect(commandStr).toContain('Start-Process $url')
    })
  })
