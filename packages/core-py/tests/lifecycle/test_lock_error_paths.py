...
def test_windows_release_ignores_oserror(lock_root: Path) -> None:
    ...
    with patch("sys.platform", "win32"):
        ...
        with patch.dict("sys.modules", {"msvcrt": mock_msvcrt}):
            ...
            # We need to mock os.open and os.fdopen to return a mock file handle that doesn't actually call msvcrt
            mock_fh = MagicMock()
            mock_fh.fileno.return_value = 123
            with patch("os.open", return_value=123), patch("os.fdopen", return_value=mock_fh):
                lock.__enter__()
