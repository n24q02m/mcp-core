"""Tests for ``sweep_stale_locks`` -- TTL + malformed-payload cleanup."""

import os
from datetime import datetime, timedelta, timezone
from pathlib import Path

from mcp_core.lifecycle.lock import sweep_stale_locks


def _write_lock_with_age(path: Path, pid: int, port: int, age_hours: float = 0) -> None:
    ts = (datetime.now(timezone.utc) - timedelta(hours=age_hours)).isoformat()
    path.write_text(f"{pid}\n{port}\ntoken\n{ts}\n", encoding="utf-8")


async def test_sweep_removes_expired_lock(tmp_path: Path) -> None:
    p = tmp_path / "demo-2001.lock"
    _write_lock_with_age(p, os.getpid(), 2001, age_hours=25)

    removed = await sweep_stale_locks("demo", ttl_hours=24, root=tmp_path)

    assert removed == 1
    assert not p.exists()


async def test_sweep_keeps_lock_within_ttl(tmp_path: Path) -> None:
    p = tmp_path / "demo-3001.lock"
    _write_lock_with_age(p, os.getpid(), 3001, age_hours=1)

    removed = await sweep_stale_locks("demo", ttl_hours=24, root=tmp_path)

    assert removed == 0
    assert p.exists()


async def test_sweep_only_targets_named_server(tmp_path: Path) -> None:
    p_demo = tmp_path / "demo-4001.lock"
    p_other = tmp_path / "other-server-4002.lock"
    _write_lock_with_age(p_demo, 999999, 4001, age_hours=25)
    _write_lock_with_age(p_other, 999999, 4002, age_hours=25)

    removed = await sweep_stale_locks("demo", root=tmp_path)

    assert removed == 1
    assert not p_demo.exists()
    assert p_other.exists()


async def test_sweep_removes_legacy_3_line_locks(tmp_path: Path) -> None:
    """Legacy format from pre-stdio-pure daemons cleaned on next sweep."""
    p = tmp_path / "demo-5001.lock"
    p.write_text("9999\n5001\ntoken\n", encoding="utf-8")  # 3 lines

    removed = await sweep_stale_locks("demo", root=tmp_path)

    assert removed == 1
    assert not p.exists()


async def test_sweep_returns_zero_when_locks_dir_missing(tmp_path: Path) -> None:
    nonexistent = tmp_path / "no-such-dir"
    assert await sweep_stale_locks("demo", root=nonexistent) == 0


async def test_sweep_handles_corrupted_lock(tmp_path: Path) -> None:
    p = tmp_path / "demo-6001.lock"
    p.write_text("garbage data not_a_pid\n", encoding="utf-8")

    removed = await sweep_stale_locks("demo", root=tmp_path)

    assert removed == 1
    assert not p.exists()


async def test_sweep_eleven_stale_locks_clears_all(tmp_path: Path) -> None:
    """Regression test for the 2026-04-28 wet-mcp pile-up scenario where
    11 stale lock files accumulated in a single session."""
    for i in range(11):
        p = tmp_path / f"wet-mcp-{50000 + i}.lock"
        _write_lock_with_age(p, 999990 + i, 50000 + i, age_hours=25)

    removed = await sweep_stale_locks("wet-mcp", root=tmp_path)

    assert removed == 11
    assert list(tmp_path.glob("wet-mcp-*.lock")) == []
