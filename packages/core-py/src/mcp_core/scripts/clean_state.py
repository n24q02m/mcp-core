...
def _clean_single_daemon(lock: Path, cache_dir: Path, verbose: bool) -> tuple[bool, bool]:
    """Clean up a single daemon's resources. Returns (killed, removed_lock)."""
    killed = False
    removed = False
...
def kill_daemons(verbose: bool = False) -> tuple[int, int]:
    ...
    removed_count = 0
    for lock in locks:
        killed, removed = _clean_single_daemon(lock, cache_dir, verbose)
        if killed:
            killed_count += 1
        if removed:
            removed_count += 1
