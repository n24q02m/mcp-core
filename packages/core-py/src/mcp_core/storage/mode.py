...
def set_local_mode(server_name: str) -> None:
    ...
    """
    write_config(server_name, {_MODE_KEY: _LOCAL_MODE_VALUE})
    logger.debug("Set local mode for %s", server_name)


def get_mode(server_name: str) -> ServerMode:
    """Get server mode.
    ...
    # Optimization: Use O(1) length and presence check instead of O(N) list comprehension
    # to avoid unnecessary memory allocation and iteration over the entire config dictionary.
    if len(config) > (1 if _MODE_KEY in config else 0):
        return "configured"

    return None


def clear_mode(server_name: str) -> None:
