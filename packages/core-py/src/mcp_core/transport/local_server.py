...
def _init_jwt_issuer(server_name: str, jwt_keys_dir: Path | None) -> JWTIssuer:
    ...
    return JWTIssuer(**jwt_issuer_kwargs)
...
def _init_oauth_app(
    ...
    """Create either a delegated or local OAuth Starlette application."""
...
def _init_mcp_handler(
    ...
    """Create MCP ASGI handler and session manager, wrapped with Bearer auth."""
...
def build_local_app(
    ...
    *,
    ...
    bearer_mcp_app, session_manager = _init_mcp_handler(
        mcp=mcp,
        jwt_issuer=jwt_issuer,
...
def _log_credential_status(
    ...
    """Decide whether to auto-open the relay form based on schema completeness."""
...
async def _run_uvicorn(
    ...
    """Run the uvicorn server with lifecycle lock and timestamp refresh task."""
    import asyncio as _asyncio
    import uvicorn
    ...
    # mistake a long-running daemon for a stale lock.
...
async def run_http_server(
    ...
    *,
    ...
    # Wire setup completion + failure callbacks.
    ...
    # Issue a long-lived access token written into the lock file
