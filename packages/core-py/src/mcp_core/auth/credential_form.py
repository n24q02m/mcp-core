...
def _get_styles() -> str:
    """Return the CSS styles for the credential form."""
    return """
        *, *::before, *::after {
...
def _get_scripts(submit_url: str) -> str:
    """Return the JavaScript logic for the credential form."""
    return f"""
        (function () {{
            ...
            function showStepInput(step) {{
                ...
                promptEl.textContent = step.prompt || "Verification Required";
                ...
                inputEl.type = step.type === "password_required" ? "password" : "text";
                inputEl.className = "field-input";
                inputEl.placeholder = step.type === "password_required" ? "Enter password" : "Enter code";
                ...
                buttonEl.addEventListener("click", function () {{
                    verifyStep(step, inputEl, buttonEl, errorEl);
                }});

                inputEl.addEventListener("keydown", function (e) {{
                    if (e.key === "Enter") verifyStep(step, inputEl, buttonEl, errorEl);
                }});

                inputEl.focus();
            }}

            function verifyStep(step, inputEl, buttonEl, errorEl) {{
                ...
                var body = {{ step_id: step.step_id }};
                body[step.field || "code"] = value;
...
def render_credential_form(
    ...
    *,
    ...
    styles = _get_styles()
    scripts = _get_scripts(submit_url_escaped)
