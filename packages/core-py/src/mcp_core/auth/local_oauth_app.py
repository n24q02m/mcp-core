...
class LocalOAuthServer:
    """OAuth 2.1 Authorization Server logic extracted into a class to reduce complexity."""
    ...
    def __init__(
        ...
        *,
        ...
        if jwt_issuer is None:
            self.jwt_issuer = JWTIssuer(server_name=server_name)
        else:
    ...
    async def otp_handler(self, request: Request) -> JSONResponse:
        ...
        # 3. Parse JSON body BEFORE incrementing attempts.
        try:
        ...
        step_sub = str(self._pending_step.get("sub", ""))
        step_context: SubjectContext = {"sub": step_sub}

        next_step: dict[str, Any] | None = None
        if self.on_step_submitted is not None:
            try:
                result = self.on_step_submitted(step_data, step_context)
                if inspect.isawaitable(result):
                    result = await result
                if isinstance(result, dict):
                    next_step = cast("dict[str, Any]", result)
        ...
        if next_step is not None and next_step.get("type") in (
            ...
            self._mark_pending_step(step_sub)
    ...
    def build_app(self) -> Starlette:
        """Build the Starlette app with routes and state."""
        routes = [
