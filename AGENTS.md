...
- PR title: same subset as the commit prefix — `feat:` or `fix:` (scope optional), lowercase subject, no agent name or emoji in front. the `pr-title` job in `.github/workflows/ci.yml` reports it red otherwise, and the squash subject it produces is what PSR reads
