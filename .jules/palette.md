...
## 2026-08-01 - The core-py/core-ts gap above was a copy, not a missing feature

**Learning:** The entry above was applied to `core-ts` and to `_username_field_html()` in `core-py`, and the flat form still rendered unlinked help text -- because `render_credential_form` carried its own inline copy of the same markup rather than calling the helper (whose docstring already claimed it was "shared by the flat + feature forms"). A duplicated template does not stay in sync; the copy silently keeps the old behaviour and reads as a language-parity gap.
**Action:** Before writing an a11y attribute into an HTML string template, check whether an extracted helper for that block already exists and whether every caller uses it. Fix by deleting the copy, not by patching it -- and assert `helper() in html` rather than asserting the attribute, since only the former fails when a copy comes back.
...
## 2026-08-07 - Add tooltips to draggable UI elements and icon-only buttons
**Learning:** Icon-only buttons (like the "x" for remove) and draggable items (like `mc-chip` with `cursor: grab`) can lack discoverability and clarity for sighted users who don't rely on screen readers. While `aria-label` handles screen reader accessibility, adding a standard `title` attribute provides a native browser tooltip on hover, significantly improving the experience for mouse users.
**Action:** When implementing draggable elements or icon-only buttons, always ensure a `title` attribute is present alongside the corresponding `aria-label` (for buttons) to provide a tooltip for sighted users.
