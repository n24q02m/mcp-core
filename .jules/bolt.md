...
## 2025-05-24 - Refactor Long Platform-Specific Functions
**Learning:** Functions that handle multiple platforms (Linux, macOS, Windows) can quickly become "overly long" and difficult to maintain. Extracting each platform's logic into dedicated private helper functions (e.g., `_get_linux_id`, `_get_darwin_id`) improves readability and makes the main function's high-level flow clearer.
**Action:** When a function contains a large `if/elif` block based on `sys.platform` or `process.platform`, extract each branch into a private helper function. This also simplifies unit testing by allowing individual platform logic to be mocked or tested in isolation if needed.
