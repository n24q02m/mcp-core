...
## 2024-06-28 - HTTP Header Parsing Hot Path Avoids Array Allocations
**Learning:** In hot paths like HTTP header parsing (e.g. `X-Forwarded-Proto` and `X-Forwarded-For`), extracting the first value from a comma-separated list via `.split(,)[0]` generates unnecessary array allocations. Using `.indexOf(,)` and `.substring()` is demonstrably faster (~2x improvement in basic tests) and reduces garbage collection pressure.
**Action:** Always prefer index-based scanning and substring extraction for parsing simple comma-separated structures in high-frequency functions.
