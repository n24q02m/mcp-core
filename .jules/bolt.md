...
## 2026-06-24 - Optimize Header Parsing
**Learning:** Using `.split(',')[0]` to parse HTTP headers like `x-forwarded-for` or `x-forwarded-proto` creates unnecessary array allocations in hot paths.
**Action:** Use `.indexOf(',')` and `.substring()` instead to extract the first value without allocating an array, especially in route middleware.
