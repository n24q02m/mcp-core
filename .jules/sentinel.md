...
## 2026-08-16 - [TOCTOU in Sensitive Directory Creation via mkdir+chmod Anti-Patterns]
**Vulnerability:** Insecure directory permissions where `mkdir(dir, { recursive: true, mode: 0o700 })` is protected against TOCTOU using `chmod(dir, 0o700)` on existing directories, but implemented via two anti-patterns: (1) wrapping it in `if (!existsSync(dir))` which misses existing overly-permissive directories, or (2) running it unconditionally on every hot path write (e.g. `put()`), causing I/O performance regressions.
**Learning:** `chmod` must execute exactly once per initialization path to properly tighten existing permissive directories without causing ongoing performance hits.
**Prevention:** Implement a module-level or class-level `_initDone` flag. Wrap the `mkdir` and `chmod` sequence inside `if (!_initDone) { ... _initDone = true }`. Avoid using `if (!existsSync)` for sensitive directory creation where `chmod` is necessary to restrict permissions.
