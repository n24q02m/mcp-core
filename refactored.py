...
def create_delegated_oauth_app(
    *,
    ...
    routes = [
        Route("/", delegated.root, methods=["GET"]),
        Route("/callback-done", delegated.callback_done, methods=["GET"]),
        Route("/login", delegated.login_get, methods=["GET"]),
        Route("/login", delegated.login_post, methods=["POST"]),
        Route("/authorize", delegated.authorize, methods=["GET"]),
        Route("/token", delegated.token, methods=["POST"]),
        Route("/register", delegated.register_handler, methods=["POST"]),
        Route("/setup-status", delegated.setup_status, methods=["GET"]),
        Route("/.well-known/oauth-authorization-server", delegated.well_known_as, methods=["GET"]),
        Route("/.well-known/oauth-protected-resource", delegated.well_known_pr, methods=["GET"]),
    ]
    if flow == "redirect":
        routes.append(Route(upstream.callback_path, delegated._callback, methods=["GET"]))
    ...
    async def _lifespan(_app: Starlette):
        try:
            ...
            for task in list(delegated._poll_tasks):
                task.cancel()
            for task in list(delegated._poll_tasks):
                try:
                    await task
    ...
    app.state.mark_setup_complete = delegated.mark_setup_complete  # type: ignore[attr-defined]
